...
def _infer_codigo_por_descricao(desc_up: str) -> Optional[str]:
    """Tenta inferir código quando o PDF perde a coluna 'Código' na extração de texto."""
    if not desc_up:
        return None
    # descontos mais comuns
    if "INSS DIFERENCA" in desc_up or "INSS DIFERENÇA" in desc_up:
        return "821"
    if "I.N.S.S" in desc_up or re.search(r"\bINSS\b", desc_up):
        return "998"
    if "DESC. CURSO" in desc_up or "DESC CURSO" in desc_up:
        return "686"
    if "ADIANTAMENTO" in desc_up:
        return "981"
    if "ATRASOS" in desc_up or "HORAS FALTAS" in desc_up:
        return "8069"
    if "PROCESSO" in desc_up:
        return "681"
    if "RESSARC" in desc_up or "PREJUI" in desc_up:
        return "255"
    if "VALE" in desc_up and "ADIANT" in desc_up:
        return "240"
    # proventos (quando faltam)
    if "DIF. MEDIA HORA FERIAS" in desc_up or "DIF. MEDIA HORA FÉRIAS" in desc_up:
        return "8189"
    if desc_up.startswith("DIF. 1/3") or "DIF. 1/3 DE FERIAS" in desc_up:
        return "8112"
    if "DIF. FERIAS" in desc_up or "DIF. FÉRIAS" in desc_up:
        return "940"
    if "SALARIO FAMILIA" in desc_up or "SALÁRIO FAMÍLIA" in desc_up:
        return "995"
    if "REFLEXO EXTRAS" in desc_up:
        return "250"
    if "REFLEXO ADIC. NOTURNO" in desc_up:
        return "854"
    return None
...
def _regex_guess(text: str) -> Dict:
    ...
    for ln in text.splitlines():
        ...
        if in_table:
            ...
            # Se o primeiro token não é código numérico, tenta inferir pelo texto
            if not re.fullmatch(r"\d{2,4}", str(codigo)):
                desc_up_full = s.upper()
                inferred = _infer_codigo_por_descricao(desc_up_full)
                if inferred:
                    codigo = inferred
                    # neste caso, descrição é a linha inteira (vamos recalcular mais abaixo)
                    toks = [codigo] + s.split()
                else:
                    continue
            ...
            if len(tail_money) == 2:
                # Situação comum: os 2 últimos "dinheiros" são (referência monetária, valor),
                # e NÃO (vencimentos, descontos).
                #
                # Exemplos:
                #  - "... 6,00 47,27"  (reflexo dsr)
                #  - "... 1,70 1,70"   (diferença / ajuste)
                #  - "... 224,42 224,42" (ressarcimento)
                #
                # Regras:
                # - se o token "referência" (último token restante) não parece referência, assumimos que o primeiro dinheiro é a referência.
                # - se o token "referência" parece referência monetária e coincide com o 1º dinheiro, também tratamos como (ref, valor).
                if referencia is None:
                    referencia = tail_money_str[0]
                    tail_money = [tail_money[1]]
                    one_value_mode = True
                else:
                    if (not _is_ref_token(referencia)) or (str(referencia) == str(tail_money_str[0])):
                        referencia = tail_money_str[0]
                        tail_money = [tail_money[1]]
                        one_value_mode = True
    ...
    # varredura suplementar: captura linhas que perderam a coluna 'Código' (pdfplumber às vezes separa o código em outra linha)
    # Ex.: "I.N.S.S 7,70 143,54" ou "DESC ADIANTAMENTO SALARIAL 500,00 500,00"
    sup_pat = re.compile(r"^(?P<desc>.+?)\s+(?P<ref>\S+)\s+(?P<val>" + MONEY_RE.pattern + r")\s*$")
    for line in (text or "").splitlines():
        sline = (line or "").strip()
        if not sline:
            continue
        mm = sup_pat.match(sline)
        if not mm:
            continue
        desc = mm.group("desc").strip()
        ref = mm.group("ref").strip()
        val = parse_money_any(mm.group("val"))
        desc_up = desc.upper()
        cod = _infer_codigo_por_descricao(desc_up)
        if not cod:
            continue
        # evita duplicar se já existe evento com mesmo código e valor
        already = False
        for e in eventos:
            if str(e.get("codigo") or "") == str(cod) and ((e.get("provento") == val) or (e.get("desconto") == val)):
                already = True
                break
        if already:
            continue
        ...
        provento = None
        desconto = None
        hint = CODE_HINT.get(str(cod))
        if hint == "D" or any(k in desc_up for k in DISCOUNT_KEYWORDS):
            desconto = val
        else:
            provento = val
...
def _post_process_eventos(eventos: List[Dict], total_venc: Optional[float] = None, total_desc: Optional[float] = None) -> List[Dict]:
    ...
    for e in eventos:
        ...
        # Se ambos preenchidos e é PROVENTO, zera desconto (corrige linhas com ref+valor interpretadas como 2 colunas)
        if provento is not None and desconto is not None and code_hint.get(codigo) == "P":
            desconto = None
