...
def generate_receipt_pdf(row: Dict, out_pdf: str, logo_path: Optional[str] = None, empresa_nome: str = "Contare", holerite_pdf_bytes: Optional[bytes] = None, holerite_page_index: int = 0) -> str:
    ...
    y -= 6*mm
    c.drawString(x, y, f"Referência 8781 (base 30): {row.get('referencia_8781') or '—'}")
    y -= 6*mm
    c.drawString(x, y, f"Remuneração Bruta proporcional: {_fmt(row.get('remuneracao_bruta_proporcional') or row.get('remuneracao_bruta_planilha') or row.get('bruto_planilha'))}")
    y -= 6*mm
