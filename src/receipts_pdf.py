...
def generate_receipt_pdf(row: Dict, out_pdf: str, logo_path: Optional[str] = None, empresa_nome: str = "Contare") -> str:
    ...
    c.drawString(x0 + 50*mm, y - 14*mm, "RECIBO COMPLEMENTAR (EXTRA-FOLHA) — BASE: HOLERITE")
    ...
    y -= 30*mm
    espelho_h = 120*mm
    _draw_box(c, x0, y, W - 2*margin, espelho_h, "Espelho do Holerite — Eventos extraídos")
    ...
    _draw_table_grid(c, x0 + 2*mm, y - 8*mm, col_w, headers, table_rows[:24], row_h=10.0)

    y -= espelho_h + 6*mm
    _draw_box(c, x0, y, W - 2*margin, 40*mm, "Cálculo do Complemento (Extra-folha)")
    c.setFont("Helvetica", 9)
    c.drawString(x0 + 2*mm, y - 10*mm, f"Bruto referencial (planilha): {_fmt_money(row.get('bruto_referencial_planilha'))}")
    c.drawString(x0 + 2*mm, y - 16*mm, f"Líquido no holerite: {_fmt_money(row.get('liquido_folha'))}")
    c.drawString(x0 + 2*mm, y - 22*mm, f"8781: {_fmt_money(row.get('verba_8781_salario_contratual'))}   |   981: {_fmt_money(row.get('verba_981_desc_adiantamento'))}   |   INSS: {_fmt_money(row.get('inss_total_desconto'))}")
    c.drawString(x0 + 2*mm, y - 28*mm, f"Pagamentos anteriores (proventos): {_fmt_money(row.get('pagamentos_anteriores_proventos'))}")
    c.drawString(x0 + 2*mm, y - 34*mm, f"Regra aplicada: {_safe(row.get('regra_aplicada'), '-')}")

    c.setFont("Helvetica-Bold", 12)
    c.drawString(x0 + 2*mm, y - 40*mm, f"VALOR LÍQUIDO A PAGAR (EXTRA-FOLHA): {_fmt_money(row.get('valor_a_pagar'))}")
