...
def generate_receipt_pdf(row: Dict, out_pdf: str, logo_path: Optional[str] = None, empresa_nome: str = "Contare", holerite_pdf_bytes: Optional[bytes] = None, holerite_page_index: int = 0) -> str:
    ...
    y -= 8*mm
    c.setFont("Helvetica", 10)
    c.drawString(x, y, f"Bruto (planilha - salário real): {_fmt(row.get('bruto_planilha'))}")
    y -= 6*mm
    c.drawString(x, y, f"Líquido holerite (CLT): {_fmt(row.get('liquido_holerite'))}")
    y -= 6*mm
    ...
    y -= 6*mm
    c.drawString(x, y, f"Outros proventos (holerite): {_fmt(row.get('outros_proventos_holerite'))}")
    y -= 6*mm
    c.drawString(x, y, f"Total descontos (holerite): {_fmt(row.get('total_descontos_holerite'))}")
    y -= 6*mm
    c.drawString(x, y, f"Líquido demonstrativo (substitui 8781): {_fmt(row.get('liquido_demonstrativo'))}")
    y -= 8*mm
    c.setFont("Helvetica-Bold", 12)
