...
if st.button("Processar", type="primary"):
    ...
    for c in colabs:
        ...
        # Novo cálculo:
        # Substitui o salário registrado no holerite (8781) pelo BRUTO da planilha
        # e mantém as demais verbas (outros proventos e descontos) do holerite.
        regra = "INDEFINIDA"
        valor_a_pagar = None
        liquido_ajustado = None
        ...
        if bruto_ref is not None and liquido is not None:
            ...
            liquido_ajustado = float(bruto_ref) + float(outros_proventos) - float(total_descontos)

            regra = "AJUSTE SALARIAL: substitui 8781 por bruto planilha; mantém demais verbas"
            valor_a_pagar = max(float(liquido_ajustado) - float(liquido), 0.0)
        ...
        rows.append({
            ...
            "liquido_ajustado": liquido_ajustado,
            "total_proventos_holerite": total_proventos,
            "total_descontos_holerite": total_descontos,
            "outros_proventos_holerite": outros_proventos,
