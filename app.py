...
if st.button("Processar", type="primary"):
    ...
    rows = []
    for c in colabs:
        ref = find_colaborador_ref(df_sal, cpf=c.get("cpf", gpt_match_fn=lambda nome, cands: gpt_disambiguate_name(nome, cands, model=openai_model, api_key=openai_key)), nome=c.get("nome"))
...
if df is not None:
    ...
    with tab2:
        idx = st.selectbox("Selecione o colaborador", df.index, format_func=lambda i: (
            f"{df.loc[i,'nome']}" + (f" — Mat:{df.loc[i,'matricula']}" if str(df.loc[i,'matricula'] or '').strip() else '') + (f" — CPF:{df.loc[i,'cpf']}" if str(df.loc[i,'cpf'] or '').strip() else '')
        ))
        row = df.loc[idx]
